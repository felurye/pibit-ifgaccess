...
void setup() {
  ...
  Serial.println("Aproxime o seu cartao do leitor...");  // ESCREVE O TEXTO NA
                                                         // JANELA SERIAL
...
void loop() {
  ...
  Serial.print(
      "UID da tag :");  // MOSTRA NA SERIAL O UID DO CARTÃO LIDO / ANOTE OS
                        // UID's DO SEU CARTÃO PLÁSTICO E DO CHAVEIRO
  ...
  for (byte i = 0; i < mfrc522.uid.size;
       i++) {  // PARA "i" IGUAL A 0, ENQUANTO "i" MENOR QUE "mfrc522.uid.size",
               // INCREMENTA "i"
    Serial.print(mfrc522.uid.uidByte[i] < 0x10
                     ? " 0"
                     : " ");  // ESCREVE O TEXTO NA JANELA SERIAL
    Serial.print(mfrc522.uid.uidByte[i],
                 HEX);  // ESCREVE O TEXTO NA JANELA SERIAL
    conteudo.concat(String(mfrc522.uid.uidByte[i] < 0x10
                               ? " 0"
                               : " "));  // CONCATENA O CONTEÚDO LIDO
    conteudo.concat(
        String(mfrc522.uid.uidByte[i], HEX));  // CONCATENA O CONTEÚDO LIDO
  }
  Serial.println();             // SALTA UMA LINHA NA JANELA SERIAL
  ...
  if (conteudo.substring(1) ==
      "86 45 75 1F") {  // TESTA SE O CARTÃO LIDO POSSUI CÓDIGO IGUAL AO DA
                        // SEQUÊNCIA "83 8E 58 02" / "83 8E 58 02" DEVE SER
                        // ALTERADO PARA O UID DO SEU CARTÃO PLÁSTICO
    digitalWrite(pino_led_liberado, HIGH);  // ACENDE O LED REFERENTE A
                                            // LIBERAÇÃO
    Serial.println(
        "Cartao1 - Acesso liberado !");    // ESCREVE O TEXTO NA JANELA SERIAL
    Serial.println();                      // SALTA UMA LINHA NA JANELA SERIAL
    delay(3000);                           // INTERVALO DE 3 SEGUNDOS
    digitalWrite(pino_led_liberado, LOW);  // APAGA O LED REFERENTE A NEGAÇÃO
  }
  if (conteudo.substring(1) ==
      "7D 42 5B D3") {  // TESTA SE O CARTÃO LIDO POSSUI CÓDIGO IGUAL AO DA
                        // SEQUÊNCIA "A3 37 1D 00" / "A3 37 1D 00" DEVE SER
                        // ALTERADO PARA O UID DO SEU CHAVEIRO
    Serial.println(
        "Cartao2 - Acesso negado !!");  // ESCREVE O TEXTO NA JANELA SERIAL
    Serial.println();                   // SALTA UMA LINHA NA JANELA SERIAL
    // Pisca o led vermelho
    for (int i = 1; i < 5;
         i++) {  // PARA "i" IGUAL A 1, ENQUANTO "i" MENOR QUE 5, INCREMENTA "i"
